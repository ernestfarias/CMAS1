...
//session_start(); //eaf2011
	...
	if($logged_in == 1)
	{
		echo	'<span class="Blanco14">ud ya tiene una session abierta,<b> '.$_SESSION['usernametext'].'</b> <a href = "logout.php">Click Aqui</a>.</span>';
...
<?php
 		$log = "si";
		$msg = "";
	if (isset($_POST['submit'])) //POSTEO EL FORM ACA Y VALIDO PSWDS
...
<? 
	...
	if ($log == "si") {
			...
			echo '<form name="frmpasomgr" action="adm_datos.php" method="post"></form><BODY onLoad=document.frmpasomgr.submit()></BODY>';
			
			
		} else {
			 echo '<form name="frmpasomgr" action="index.php" method="post">					   	
			    	</form>
					<BODY onLoad=document.frmpasomgr.submit()>
					</BODY>';
		}
...
<?
	...
	} //FIN DE SUBMITIO EL FORM
	else //NO SUBMITIO EL FORM MUESTRA LA PANTALLA DE LOGIN
...
<form action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
 ...
 //	    <input type="submit" name="submit" value="INGRESAR" style="font-size:8pt;cursor:pointer;margin:0px;padding:0px;border:0;background-color:#000000;color:white" >
    ...
    <input type="submit" name="submit" value="Ingreso" style="" >
