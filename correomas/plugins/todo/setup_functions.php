...
function todo_show_reminders_do() {
	...
	if (($todo_things == true) && ($todo_show_left_todos == 1)) {
		...
		'&nbsp;<img src="../images/icons/todo.gif" width="24" height="21" border="0" align="absmiddle" vspace="0" hspace="3">&nbsp;<b><a href="../plugins/todo/todo.php" target="right">' . _("To Do") . '</a><b></td></tr>' .
		// Linha separadora
		html_tag( 'tr' ) .
		...
		todo_init($todos);
		$ntodos = todo_count($todos);
		echo '<table border="0" width="100%"><tr><td><strong>' . _("Subject")
		  . '</strong></td><td align="right"><strong>' . _("Deadline") . "</strong></td></tr>\n";
		for($i=0;$i<$ntodos;$i++) {
			$deadline = todo_get_todo_dl($todos, $i); 
			$title = todo_get_todo_title($todos, $i);
		...
		echo '</td></tr></table></td></tr><tr><td bgcolor=808080 height=1></td></tr></table>';
	}
}
