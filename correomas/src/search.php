...
if (isset($composenew) && $composenew) {
    ...
    displayPageHeader($color, 'None', "comp_in_new('$comp_uri');", false,_('Search'));
} else {
    displayPageHeader($color, 'None', '' , '',_('Search'));
}
...
// Retirado pra ficar no padrão do Outlook
/* echo html_tag( 'table',
        html_tag( 'tr', "\n" .
...
if ($saved_count > 0) {
    echo "<br />\n"
    . html_tag( 'table', '', 'center', $color[9], 'width="100%" cellpadding="1" cellspacing="1" border="0"' )
    . html_tag( 'tr',
...
/* Search Form */
// This point layout is changed for equal outlook 2003
// Inicia o table
echo "<br>";
echo html_tag( 'table', '' , 'center' ,'#808080', 'width="100%" border="0" cellpadding="0" cellspacing="1"')
	. '<form action="search.php" name="s">'
	. html_tag( 'tr' )
	. html_tag( 'td', '', '', '#D4D0C8','' );
		echo html_tag( 'table' , '', '', '', 'width="100%" border="0" cellspacing="0" cellpadding="0"' )
		. html_tag( 'tr' )
		. html_tag( 'td', _('Search') . ':&nbsp;', 'right', '','widht="10%" height="22"' );
		// FIXME: explain all str_replace calls.
		$what_disp = str_replace(',', ' ', $what);
		$what_disp = str_replace('\\\\', '\\', $what_disp);
		$what_disp = str_replace('\\"', '"', $what_disp);
		$what_disp = str_replace('"', '&quot;', $what_disp);
		echo html_tag( 'td', '<input type="text" size="20" name="what" value="' . $what_disp . '" />' . "\n", 'left', '', 'widht="20%"' )
		. html_tag( 'td', _('Search in') . ':&nbsp;', 'right', '','widht="12%"' )
		. html_tag( 'td', '', 'left','','widht="19%"' )
			. '<select name="mailbox">'
			...
			echo '</select>'.
		"</td>\n";
		echo html_tag( 'td',  _('Field') . ':&nbsp;', 'right', '','widht="8%"' );
		echo html_tag( 'td', '', 'left','','widht=18%' )
			. "<select name=\"where\">";
			...
			echo "</select>\n" .
			"</td>\n";
		echo html_tag( 'td', '<input type="submit" name="submit" value="' . _("Search") . '" />' . "\n", 'left', '', 'widht=13%' );
...
$what_disp = str_replace('\\"', '"', $what_disp);
$what_disp = str_replace('"', '&quot;', $what_disp);
echo html_tag( 'td','Procurar:&nbsp;','right','','widht="10%" height="22"');
