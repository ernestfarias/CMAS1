<? session_start();  
//PORQUERIA PARA QUE NO MOLESTE EL REG GLOBALS del php.ini
...
//FIN PORQUERIA PARA QUE NO MOLESTE EL REG GLOBALS

//DEBUGGER1
/*
echo 'DEBUGGER<font color="#FFFFFF" size=2>VARIABLES DE SESSION:<BR>';
print_r($_SESSION);
...
if  (!empty($_POST)) //si se posteo algo, cargo las variables y seteo las de session
{
...
if ($barriost== "") {$barriost="TODOS";} //SI BARRIOS LLEGA VACIO LE PONGO TODOS
if (isset($ciudadst)) {$_SESSION["ciudadst"] = $ciudadst;}
if (isset($barriost)) {$_SESSION["barriost"] = $barriost;}
...
if(isset($barriost) && !empty($barriost))   //lcargao las varas para barrio y ciudad
{
		if ($barriost=='TODOS')
			 			{
							$barriost = " ";
		       	}		else {	$barriost = " AND barrio='$barriost' ";}
	
			if ($ciudadst=='TODAS') //si esta todas las ciudad por defecto estarian tambien todos los barrios
			    { 
					 	$ciudadst = " ";
						$barriost = " ";
		    	}else{	$ciudadst = " AND localidad='$ciudadst' ";}			
}
else //el else este es util cuando pone en TODAS las ciudaddes porque el barriost llega con NULL, porque op barrios no se postea pq queda disabled
{
		if ($barriost=='TODOS')
			 			{
							$barriost = " ";
...
$rscant= @mysql_query($sqlcant) or die (mysql_error()."SQLCANT----->>>".$sqlcant."vars:<br>ciudadst:".$ciudadst."<br>barriost:".$barriost."<br>userinputst:".$userinputst."<br>claselugarst:".$claselugarst); 
...
//echo $sqlbusqueda; //DEBUGGER!!
		...
		$rsbusqueda = @mysql_query($sqlbusqueda.$sqllimit) or die (mysql_error()."SQLBUSQUEDA----->>>".$sqlbusqueda."vars:<br>ciudadst:".$ciudadst); 
...
<title>ComerMas.com.ar - Lugares para comer - Tenedor libre - Restaurants - Bares en <?=$donde?></title>
...
<table width="779" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="ae5959">
  ...
  <td height="20" width="317" bordercolor="#FFFFFF"><b><a href="detalles.php?id=<?=$id?>" target="_blank"><?=ucwords($nombre)?></a></b><span class="Naranja10"> (<?=$clase?>)</span></td>
					...
					<? } else {?>
					            <tr bgcolor="<? echo $varcolor?>" class="Blanco10"> 
  <td height="20" colspan="4" bordercolor="#FFFFFF"><span class="Naranja10t"><b><a href="detalles.php?id=<?=$id?>" target="_blank">
    <?=ucwords($nombre)?>
  </a></b> (
    <?=$clase?>
	  	...
	  	recordsetNav($mysql_link,$sqlbusqueda,'busqueda.php',$qry ,$offset,$limit,'85%',1,1); 
...
<script type="text/javascript" src="http://www.google.com/afsonline/show_afs_search.js"></script>
            ...
            <center>
              ...
              su comercio a la base de ComerMas(gratis)</span></a> <span class="Naranja10"><br>
              </span><br>
